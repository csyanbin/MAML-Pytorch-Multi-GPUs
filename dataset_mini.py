...
class dataset_mini(object):
    def __init__(self, n_examples, n_episodes, split, args):
        ...
        self.seed       = args['seed']
        self.root_dir   = '/mnt/aitrics_ext/ext01/yanbin/MAML-Pytorch-Multi-GPUs/data/miniImagenet'
...
import torch
def get_data(loader, batchsz=4, n_way=5, n_shot=1, n_query=15):
    support_x, support_y, meta_x, meta_y = [],[],[],[]
    for b in range(batchsz):
        support, s_labels, query, q_labels, unlabel = loader.next_data(n_way, n_shot, n_query)
        support = np.reshape(support, (support.shape[0]*support.shape[1],)+support.shape[2:])
        support = torch.from_numpy(np.transpose(support, (0,3,1,2))) # -->nxwxhx3
        ...
        support_y.append(s_labels)
        meta_x.append(query)
        meta_y.append(q_labels)
    
    support_x = torch.stack(support_x,dim=0)
    support_y = torch.stack(support_y,dim=0)
    meta_x = torch.stack(meta_x,dim=0)
    meta_y = torch.stack(meta_y,dim=0)
